...
- `PB_VERSION`: mặc định `0.28.2`.
- `LITESTREAM_INIT_MODE`: `true` cho lần chạy đầu (khởi tạo DB mới), sau đó trả về `false`.
- `LITESTREAM_S3_PATH`: mặc định `pocketbase/data.db`.
...
- Kiểm tra endpoint health: `/api/health`.
